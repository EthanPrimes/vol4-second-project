...
\par However, boats with triangular sails work very differently from boats with square sails. Instead of being pushed by the wind, triangular sails act similar to airplane wings: the wind blows faster over their front than their back, causing a pressure differential that \textit{pulls} the boat in the direction the sail is facing. This allows boats with triangular sails to sail diagonally upwind. However, since the sail is assymetric, this also makes sailing directly downwind very unstable.

\par This new dynamic can be neatly added to our model of Zermelo Navigation (Model 1) by making velocity a function of the wind direction and the boat's heading. We created a velocity function that matches sailing speeds respecting wind direction, wind velocity, and boat heading.
...
\par Given the dynamics described in \ref{sec:model3}, the Hamiltonian is given by 
...
\par We implemented this formulation as a boundary value problem using \texttt{scipy.solve\_bvp()}. However, since the optimal solution dis-continually between headings while going upwind or downwind, the solution has sharp corners which break \texttt{scipy.solve\_bvp()}, which expects smooth solutions.
