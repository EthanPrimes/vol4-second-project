...
We begin by solving the standard Zermelo Navigation problem, where 

We chose $\theta,$ the angle between the $x$-axis and our heading, to be our control, and assumed that the velocity of the boat would be given by
\[
\begin{bmatrix} u_1 \\ u_2 \end{bmatrix} = v \begin{bmatrix} \cos(\theta - \phi) \cos(\theta) \\ \cos(\theta - \phi) \sin(\theta) \end{bmatrix},
\]
where the $\cos(\theta - \phi)$ refers to the exposure of the sail to the wind. This model assumes that the boat moves in the direction that it's facing with speed proportional to the the cosine of the angle between the sail and the wind. This yields the following:
\[
\begin{bmatrix} \dot x \\ \dot y \end{bmatrix} = v \begin{bmatrix} \cos(\theta - \phi) \cos(\theta) \\ \cos(\theta - \phi) \sin(\theta) \end{bmatrix} + \begin{bmatrix} w_1(x, y) \\ w_2(x, y) \end{bmatrix}.
\]
...
\columnbreak
\begin{center}
\LARGE{\textbf{Non-Constant Wind and Current}}
\end{center}
